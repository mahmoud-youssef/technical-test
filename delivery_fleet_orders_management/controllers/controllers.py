...
import requests
from odoo import http, _, exceptions, models
...
class OdooAPI(http.Controller):
    ...
    def authenticate(self, *args, **post):
        ...
        user = request.env['res.users'].sudo().search([('login', '=', login_name)], limit=1)
        user.update({
            'user_token_api': rand_token
        })
        ...
        try:
            ...
            user["result"]["user_token_api"] = rand_token
        except Exception:
    ...
    def call_model_function(self, model, function, **post):
        if "token" in post:
            user = request.env['res.users'].sudo().search([('user_token_api', '=', post["token"])], limit=1)
            if not user:
                raise exceptions.AccessDenied(message='UnAuthorize')
        else:
            raise exceptions.AccessDenied(message='UnAuthorize')
    ...
    def call_obj_function(self, model, rec_id, function, **post):
        ...
        if function == 'action_confirm':
            obj.action_confirm()
            obj._create_invoices()
            obj2 = request.env[model].browse(rec_id).ensure_one()
            x = len(obj2.invoice_ids)
            return obj2.invoice_ids[x - 1].id, obj2.name
        else:
            result = getattr(obj, function)(*args, **kwargs)
            return result

    @http.route(
        '/api/data/<string:model>/<int:rec_id>/<string:function>',
        type='http', auth='user', methods=['GET'], csrf=False)
    def test(self, model, rec_id, function):
        kwargs = {}
        obj = request.env[model].browse(rec_id).ensure_one()
        if function == 'action_confirm':
            obj.action_confirm()
            ...
            res = {
                "result": obj2.invoice_ids[x - 1].id,
                "origin": obj2.name
    ...
    @http.route(
        '/api/domain/<string:model>/<string:domain>/',
        type='http', auth='user', methods=['GET'], csrf=False)
    def get_model_data1(self, model, domain, **params):
        try:
        ...
        if domain:
            x = ast.literal_eval(domain)

            records = request.env[model].search(x, order=orders)
    ...
    def get_model_data(self, model, **params):
        # Token
        # if "token" in params:
        #     user = request.env['res.users'].sudo().search([('user_token_api', '=', params["token"])], limit=1)
        #     if not user:
        #         raise exceptions.AccessDenied(message='UnAuthorize')
        # else:
        #     raise exceptions.AccessDenied(message='UnAuthorize')
        ...
        if "group_by" in params:
            group_by_list = json.loads(params["group_by"])
        else:
            res = {
                "count": len(records),
                "prev": prev_page,
